...
from ModalityFusioner_flow import ModalityFusioner, LanguageConnecter, FlowActionModel
import numpy as np
...
def train(rank, args):
    ...
    if dataset_name not in {"ABC_D", "ABCD_D", "task_debug", "debug"}:
    ...
    if training_mode == "pretrain":
        dataset = PretrainCalvinDataset(
            dataset_path=f"/media/yang/22BC8B99BC8B665F/UBUN/U/RoboBERT/DATA/{dataset_name}", # "/home/wibot/Data/SC/{dataset_name}",
            image_processor=image_processor,
            obs_horizon=obs_horizon,
            pred_horizon=pred_horizon,
        )
        ...
        for epoch_idx in tglobal:
            ...
            with tqdm(dataloader, desc='Batch', leave=False) as tepoch:
                for nbatch in tepoch:
                    ...
                    t = torch.rand(rel_actions.size(0))

                    a_t, a_0 = rf.create_flow(rel_actions, t)
                    ...
                    a_t = a_t.to(device)
                    a_0 = a_0.to(device)
                    if training_mode == "second" and dataset_name == "task_debug": # "ABC_D":
                        rgb_static, rgb_gripper, rel_actions, language_x = my_mixup([rgb_static, rgb_gripper, rel_actions, language_x])
                    
                    v_pred = model(rgb_static, rgb_gripper, a_t, language_x, t=t)
 
                    # L2 loss
                    loss = rf.mse_loss(v_pred, rel_actions, a_0)
            ...
            if rank == 0:
                torch.save({
                    ...
                    f"/media/yang/22BC8B99BC8B665F/UBUN/U/RoboBERT/ckpt/model-{training_mode}-{dataset_name}.pt") #de -{epoch_idx}-{training_mode}-{dataset_name}.pt")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--world_size', type=int, default=1, help="the number of GPUs in DDP training, assuming there are two GPUs")
    parser.add_argument('--training_mode', type=str, default="second", help="the training stage string, only support 'first' and 'second'")
    parser.add_argument('--dataset_name', type=str, default="debug")
    parser.add_argument('--checkpoint', type=str, default="/media/yang/22BC8B99BC8B665F/UBUN/U/RoboBERT/ckpt/model-first-debug.pt", help="the checkpoint path for second stage training")
