...
from my_models.DDP_training.flow import RectifiedFlow
...
from ModalityFusioner_flow import ModalityFusioner, LanguageConnecter
...
class GPTAgent(CalvinBaseModel):
    ...
    def step(self, obs, language):
        ...
        if self.action_output_num == 0:
            ...
            step = 50
            dt = 1.0 / step
            a_t = torch.randn((1, self.pred_horizon, 7), device=self.device)
            for j in range(step):
                t = j * dt
                t = torch.tensor([t]).to(self.device)
                v_pred = self.actor['v_net'](
                            sample=a_t,
                            t=t,
                            global_cond=obs_cond
                        )
                a_t = rf.euler(a_t, v_pred, dt)
